...
// судя по логике, эта функция должна возращать функцию, которая:
// принимает на вход отношение прошедшего с начала анимации времени к ее длительности
// возращает прогресс от 0 до 1
// оригинальная функция на входе принимает координаты точек кривой Безье
// суть слишком математична, разобраться нереально
const bezierEasing = (mX1, mY1, mX2, mY2) => {
  ...
  // тут задаются разные методы этой функции

  // получить контрольные точки кривой Безье
  ...
  // получить строку со значениями аргументов, переданных в функцию
  f.toString = function () {
    return str;
  };

  const css = `cubic-bezier(` + args + `)`;
  // получить css свойство со значением кривой Безье
  f.toCSS = function () {
    return css;
  };
