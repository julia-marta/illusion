...
class Stages3DView extends AbstractView {
  ...
  async load() {
    // загружаем материалы и получаем конфиг с ними
  ...
  add3dStages() {
    ...
    // Добавляем пробел между stage3 и stage5, поскольку на stage4 у нас анимация с плакатом и китом
    stages.push(null);
    this.stage4 = null;
    ...
    // Добавляем пробел под последний раздел, поскольку на stage6 у нас анимированная карта мероприятия
    stages.push(null);
    this.stage6 = null;
    this.stages = stages;
  ...
  async loadMaterials() {
    // получаем map с готовыми текстурами для Matcap
    const textures = await loadTextures(getMatcapTexturesPaths());
    // создаём конфиг с материалами, получаем готовые Matcap материалы
    const config = {
  ...
  updateCamera(width, height) {
    ...
    // обновление размера (ориентации) и настроек сцен в соответствии с этим
  ...
  // отрисовка (метод родительского класса)
  render() {
  ...
  // запуск анимаций (метод родительского класса)
  play() {
  ...
  changeStateTo(stateName, substage, onComplete, enforce = false) {
    ...
    // 1) Вращаем сцену с подразделами, чтобы вывести на экран нужный объект
    const {stage5} =this;
