...
class AbstractView {
  ...
  // метод установления дефолтной функции рендера
  setDefaultRender() {
    this.renderFunc = () => {
  ...
  // метод установления кастомной функции рендера
  ...
  play() {
    ...
    const onAnimationFrame = (target) => {
      ...
      if (target.resizeInProgress) {
        target.resize();
      }
      // Рендерим
      target.render();

      // Сбрасываем флаг, чтобы в следующем фрейме не выполнять лишних действий
      target.resizeInProgress = false;

      // Проверяем, нужно ли продолжать перезапускать requestAnimationFrame
      if (!target.started || !target.playing) {
        return;
      }
